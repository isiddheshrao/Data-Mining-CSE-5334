...
class LogisticRegression:
    ...
    def learn(self, X, target):
        ...
        for self.iteration_count in range(self.epochs):
            ...
            current_loss = self.cross_entropy_loss(prediction, target)
            ...
            if (gradient == np.zeros(X.shape[1])).all():
                print("Cross - Entropy loss at epoch %s: %s" % (self.iteration_count + 1, self.cross_entropy_loss(prediction, target)))
